...
// Prompt for breaking down model's answer into statements
...
// Prompt for evaluating each statement's relevance
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const statements = statementsResult.statements.length === 0 
    ? [opts.submission] 
  ...
  const { object: evaluationResult } = await generateObject({
    ...
    schema: z.object({
      verdicts: z.array(
        z.object({
          verdict: z.enum(["yes", "no", "unsure"]).describe("Relevance verdict"),
  ...
  const statementScores = evaluationResult.verdicts.map(verdict => scores[verdict.verdict]);
